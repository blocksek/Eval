...
class RedisDatabase(Database):
    ...
    def count_team_instances(self, team_id: str) -> int:
        keys = self.__client.keys(f"instance/chal-*-{team_id}")
        return len(keys)

    def get_team_instance_names(self, team_id: str) -> list:
        keys = self.__client.keys(f"instance/chal-*-{team_id}")
        # Extract challenge name from "instance/chal-{challenge}-{team_id}"
        names = []
        for key in keys:
            # Remove "instance/chal-" prefix and "-{team_id}" suffix
            name = key[len("instance/chal-"):-len(f"-{team_id}")]
            names.append(name)
        return sorted(names)
